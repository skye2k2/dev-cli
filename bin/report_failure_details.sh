...
if [ "$TRAVIS_PULL_REQUEST" == false ] && [ "$TRAVIS_BRANCH" == "master" ]; then
  ...
  if [[ ! -z "$STATUS_MESSAGE" ]]; then
    ...
    curl -X POST --data-urlencode 'payload={ "username": "TravisBot", "text": "'"$STATUS_TITLE"'", "mrkdwn": true, "attachments": [{ "fallback": "'"$STATUS_TITLE_FALLBACK"'", "color": "'"$COLOR"'", "mrkdwn_in": ["text"], "text": "'"$STATUS_MESSAGE"'" }]}' $SLACK_WEBHOOK_URL
  fi
fi
